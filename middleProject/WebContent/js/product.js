$(document).ready(function(){
  ...
  $('.colors-large a').click(function(){return false;});
	...
	$('.sizes a span, .categories a span').each(function(i, el){
		...
		$(el).parent().on('click', function(){
			...
			if($(this).hasClass('checked')){				
				...
				var lprod = $(this).attr('id');
				$('.product>input[value!='+lprod+']').parent().css('display','');
				return false;
			}
			
			$('.lprodCheck a').each(function(i,v){
				$aaa = $(this);
				if($(this).hasClass('checked')){
					console.log(i , this.tagName);
					$(this).find('.y').removeClass('animate');	
					//setTimeout(function(){
					$(this).find('.x').removeClass('animate');							
					//}, 50);	
			...
			$(this).find('.x').addClass('animate');		
			setTimeout(function(){
				$elparent.find('.y').addClass('animate');
			}, 100);	
			$(this).addClass('checked');
			
			var lprod = $(this).attr('id');
			$('.product>input[value!='+lprod+']').parent().css('display', 'none');
	...
	$('.add_to_cart').click(function(){
		...
		var price = $(this).siblings('.prodPrice').val();
		var orderId = $('#orderId').val();
		var prodId = $(this).siblings('.prodId').val();
		$.ajax({
			url : '/middleProject/cart/insertDetail',
			data : {'prod_id' : $(this).siblings('.prodId').val(),
					'order_id' : orderId,
					'del_quanity' : '1',
					'del_price' : price
					},
		...
		setTimeout(function(){
			...
			var cartItem = "<div class='cart-item'><div class='img-wrap'><input type='hidden' class = 'cartProdId' value='"+prodId+"'><img src='"+productImage+"' alt='' /></div><span>"+productName+"</span><strong>"+price+"</strong><div class='cart-item-border'></div><div class='delete-item'></div></div>";			

			$("#cart .empty").hide();			
			$("#cart").append(cartItem);
			$("#checkout").fadeIn(500);
			
			$("#cart .cart-item").last().addClass('flush')
				.find(".delete-item").click(function(){
					$(this).parent().fadeOut(300, function(){
						$(this).remove();
						if($("#cart .cart-item").size() == 0){
					...
					$.ajax({
						url : '/middleProject/cart/detailDelete',
						data : {'prod_id' : $(this).parent().find('.cartProdId').val(),
	...
	$('#checkout').on('click', function() {
		location.href = "/middleProject/view/cart";
	});
